...
B1= params["Other Area _B1"]                
B2= params["Other Adapter Area_B2"]             
A_IO= params["Tx or Rx Area _A_IO_buffer"]
d1= params["Tx Adapter Area_d1"]
d2= params["Other Tx Adapter Area_d2"]
d3= params["Rx Adapter Area_d3"]
d4= params["Other Rx Adapter Area_d4"]
validate =False

#Power coefficients is in uW
Y1= params["Y2_wr_clk"]
Y2= params["Y2_rd_clk"]
Y3= params["Y2_ns_fwd_clk"]
Y4= params["Y2_fs_fwd_clk"]

Y5_1= params["Y1_in_tx_clk"]
Y5_2= params["Y1_wr_clk"]
Y5_3= params["Y1_ns_fwd_clk"]

Y6_1= params["Y1_in_rx_clk"]
Y6_2= params["Y1_rd_clk"]
Y6_3= params["Y1_fs_fwd_clk"]
...
def area_aib(Len, mode, n_Tx, n_Rx):
    ...
    #Adapter Area
    A_adpt_inital= d1*n_Tx+d2+d3*n_Rx+d4+B2     
    A_other = B1
...
def performance_aib(volt, n_Tx, n_Rx):
    ...
    P_Tx_adpt=Y1*noc_wr_clk+(Y3)*aib_ns_fwd_clk
    P_Rx_adpt=(Y2)*noc_rd_clk+(Y4)*aib_fs_fwd_clk
    P_Tx_adpt+=(Y5_1*n_Tx*8)*aib_adapt_max_txclk+(Y5_2*n_Tx*8)*noc_wr_clk+(Y5_3*n_Tx*2)*aib_ns_fwd_clk
    P_Rx_adpt+=(Y6_1*n_Rx*8)*aib_adapt_max_rxclk+(Y6_2*n_Rx*8)*noc_rd_clk+(Y6_3*n_Rx*2)*aib_fs_fwd_clk
