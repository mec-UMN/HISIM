...
def create_default_files(G, TYPE_DEFAULT_FILES):
    # The generic templates below are component-oriented.  A plain 3D mesh is
    # the homogeneous, one-stack form of the user-defined mapper, where the
    # configured chip count means tiers per stack.  Routing it through the
    # already-tested mapper keeps 3D from falling into a partially initialised
    # branch (the old code left ``chip_increment``/``chip_count`` undefined).
    if TYPE_DEFAULT_FILES == "3D_Mesh":
        configured_chip_count = globals()["chip_count"]
        return create_files_from_user_input(G, 1, max(2, int(configured_chip_count)), tile_count_dict)
