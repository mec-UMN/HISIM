...
def area_router_single(channel_width, type):
    ...
    """
    if DEBUG:
        #piechart of all area components
        #print({'channelArea':channelArea,'inputArea':inputArea,'switchArea':switchArea,'outputArea':outputArea})
        labels = ['Channel Area', 'Input Buffer Area', 'Switch Area', 'Output Buffer Area']
        sizes = [channelArea, inputArea, switchArea, outputArea]
        fig1, ax1 = plt.subplots()
        ax1.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90)
        #label size value on pie chart
        for i, size in enumerate(sizes):
            ax1.text(0, 0, f'{size:.2f}', ha='center', va='center')
        ax1.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
        plt.title('Area Breakdown')
        plt.savefig('area_breakdown_router.png')   
    """ 
...
def power_router_single(channel_width,type,latency_breakdown):
    ...
    """
    if DEBUG:
        #piechart of all power components
        labels = ['Channel Wire Power', 'Channel Clock Power', 'Channel DFF Power', 'Input Read Power', 'Input Write Power', 'Switch Power', 'Switch Control Power', 'Output Power', 'Output Clock Power', 'Output Control Power']
        sizes = [channel_wire_power, channel_clk_power, channel_DFFPower, inputReadPower, inputWritePower, switchPower, switchPowerCtrl, outputPower, outputPowerClk, outputCtrlPower]
        fig1, ax1 = plt.subplots()
        ax1.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90)
        ax1.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
        plt.title('Power Consumption Breakdown')
        plt.savefig('power_breakdown_router.png')
    """
