import math
from Module_1_Compute.HISIM_2_0_Files.Mem import area_mem_tile, latency_buf, determine_buffer_energy, nearest_ceil_value
import pandas as pd
import numpy as np
...
def latency_sa_tile(sa_df,compute_json_data, tile_latency, tile_latency_breakdown):
    ...
    for _, row in sa_df.iterrows():
        ...
        SA_size_y=row["SA_size_y"]
        n_SA=row["n_SA"]
        layers_mapped=list(row["layer_macs"].keys())  
        tile_latency[chip_tile_idx]=0      
        #import pdb; pdb.set_trace()
        for layer_idx in layers_mapped:
            util={}
            no_inp_cycles=1
            for ele in ["SA_size_x", "SA_size_y", "n_SA"]:
                no_inp_cycles*=math.ceil(row["wrap_"+ele][layer_idx]/row[ele])
                util[ele]= row["wrap_"+ele][layer_idx]/row[ele] if row[ele]>row["wrap_"+ele][layer_idx] else 1

            no_hw_macs = no_inp_cycles*reduce(operator.mul, [util[i] for i in util], 1)* n_SA * SA_size_x * SA_size_y* SA_size_x
            #import pdb; pdb.set_trace()
            #print("row", row, no_hw_macs)
            #print("row", row, no_hw_macs)
            no_inp_cycles*=math.ceil(row["layer_macs"][layer_idx]/no_hw_macs) #to incorporate dimension assigned to Auto
            no_inp_cycles*= row["wrap_Temporal"][layer_idx]
            total_max_macs= no_inp_cycles* n_SA * SA_size_x * SA_size_y* SA_size_x
            total_layer_macs= row["layer_macs"][layer_idx]*row["wrap_Temporal"][layer_idx]
            util["avg"]= total_layer_macs/total_max_macs 
            ...
            tile_latency_layer=max(latency_pe,max(latency_buf_rd.values()), latency_accum)*no_inp_cycles/row["Clock Frequency (Hz)"]
            tile_latency[chip_tile_idx]+=tile_latency_layer
            compute_latency[chip_tile_idx]=tile_latency[chip_tile_idx]
            tile_latency_breakdown[chip_tile_idx][layer_idx]={"latency_pe": latency_pe*no_inp_cycles/row["Clock Frequency (Hz)"],
                                                ...
                                                "tile_latency_layer": tile_latency_layer}
...
def energy_sa_tile(sa_df,compute_json_data,tile_energy,tile_latency_breakdown, tile_latency,tile_buf_df, lut_df):
    ...
    for _ , row in sa_df.iterrows():
        ...
        SA_size_y=row["SA_size_y"]
        n_SA=row["n_SA"]
        tile_energy_breakdown[chip_tile_idx]={}
        tile_energy[chip_tile_idx]=0
        for layer_idx in row["layer_macs"].keys():
            #PE
            ...
            power_pe*= n_SA
            energy_pe = power_pe*tile_latency_breakdown[chip_tile_idx][layer_idx]["latency_pe"]*tile_latency_breakdown[chip_tile_idx][layer_idx]["F_actpes"]*tile_latency_breakdown[chip_tile_idx][layer_idx]["util"]["SA_size_x"]*tile_latency_breakdown[chip_tile_idx][layer_idx]["util"]["SA_size_y"]*tile_latency_breakdown[chip_tile_idx][layer_idx]["util"]["n_SA"]
            #Ctl
            power_ctl=(compute_json_data["Power_ctl_a"]*SA_size_y+compute_json_data["Power_ctl_b"])*n_SA
            #import pdb; pdb.set_trace()
            energy_clt=power_ctl*tile_latency_breakdown[chip_tile_idx][layer_idx]["tile_latency_layer"]
            ...
            for buf_type in ["inbuf", "wgbuf", "outbuf"]:
                ...
                energy_buf += determine_buffer_energy(lut_row["P_freq_uW_MHz_read"], lut_row["P_freq_uW_MHz_write"],row["Clock Frequency (Hz)"], lut_row["K_lut_read"], lut_row["K_lut_write"], row_buf["F_actbitlines"], row_buf["F_non_zeros_r"],row_buf["F_non_zeros_w"], tile_latency_breakdown[chip_tile_idx][layer_idx]["latency_buf_rd"][buf_type], 0)

            #Accu
            energy_accum = compute_json_data["energy_per_adder"]*row["prec"]*row["num_adders_tile"]*tile_latency_breakdown[chip_tile_idx][layer_idx]["latency_accum"]
            ...
            tile_energy_breakdown[chip_tile_idx][layer_idx]={"energy_pe": energy_pe,
