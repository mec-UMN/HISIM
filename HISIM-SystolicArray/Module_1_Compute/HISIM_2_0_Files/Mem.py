...
def latency_ddr(compute_json_data, row, idx):
    ...
    if ddr_config_name not in ddr_config:
        ...
        sys.exit(1)
    next_layer_type=row["Next Layer Type"][idx]
    #read
    #print(row)
    ddr_wrap_size = row["mem_req_layer"][idx]
    N_raccess_rd= math.ceil(ddr_wrap_size/ddr_config[ddr_config_name]["ddr_bus_width_burst"])*row["ddr_reuse_mul_r"][idx]
...
def latency_mem_tile(mem_df, compute_json_data, tile_latency,tile_latency_breakdown):
    ...
    for _, row in mem_df.iterrows():
        ...
        if row["HW Type"]!="DDR":
            ...
            ddr_latency[chip_tile_idx]=0
            for idx in row["Accesses_r"].keys():
                #print(NW, NB, Nbank, CM)
                #import pdb; pdb.set_trace()
                SRAM_read_latency=latency_buf(compute_json_data,  row["Accesses_r"][idx])/row["Clock Frequency (Hz)"]
                SRAM_write_latency=latency_buf(compute_json_data,  row["Accesses_w"][idx])/row["Clock Frequency (Hz)"]
                tile_latency_breakdown[chip_tile_idx][idx]={"SRAM_read_latency": SRAM_read_latency, 
                ...
                if row["DDR_access_layer"][idx]:
                    latency_rd_ddr, latency_wr_ddr, N_accesses_ddr,bus_width_ddr=latency_ddr(compute_json_data, row, idx)
                    tile_latency_breakdown[chip_tile_idx][idx].update({"DDR_read_latency": latency_rd_ddr, "DDR_write_latency": latency_wr_ddr, "DDR_accesses": N_accesses_ddr, "DDR_bus_width": bus_width_ddr})
                    ddr_latency[chip_tile_idx]+=latency_rd_ddr+latency_wr_ddr
                else:
                    tile_latency_breakdown[chip_tile_idx][idx].update({"DDR_read_latency": 0, "DDR_write_latency": 0, "DDR_accesses": 0, "DDR_bus_width": 0})
...
def energy_mem_tile(mem_df, compute_json_data, tile_energy, tile_latency_breakdown, lut_df):
    ...
    for _, row in mem_df.iterrows():
        ...
        if row["HW Type"]!="DDR":# to avoid duplicate calculation for DDR
            N_ddr_accesses=sum([tile_latency_breakdown[chip_tile_idx][idx]["DDR_accesses"] for idx in tile_latency_breakdown[chip_tile_idx]])
            ddr_energy[chip_tile_idx]= (ddr_config[ddr_config_name]["ddr_energy_per_bit"]*N_ddr_accesses)
            #print(lut_row)
        tile_energy[chip_tile_idx]=0
        for key, val in tile_latency_breakdown[chip_tile_idx].items():
            #import pdb; pdb.set_trace()
            tile_energy[chip_tile_idx]+=determine_buffer_energy(lut_row["P_freq_uW_MHz_read"], lut_row["P_freq_uW_MHz_write"],row["Clock Frequency (Hz)"], lut_row["K_lut_read"], lut_row["K_lut_write"], row["F_actbitlines"], 0.5, 0.5, val["SRAM_read_latency"], val["SRAM_write_latency"])
...
def determine_buffer_energy(P_freq_uW_MHz_read, P_freq_uW_MHz_write, Clock_Frequency_Hz, K_lut_read, K_lut_write, F_actbitlines, F_non_zeros_r, F_non_zeros_w, SRAM_read_latency, SRAM_write_latency):
    # ``lut_row[... ]`` is normally a one-row Series, but its dtype depends on
    # the pandas backend.  In particular, a calibrated ``TBC`` column can be a
    # nullable string Series, so assigning a float into ``.values[0]`` raises a
    # TypeError with pandas' Arrow string dtype.  Read the scalar and apply the
    # fallback locally instead of mutating the caller's DataFrame/Series.
    def scalar(value):
    ...
    p_read = scalar(P_freq_uW_MHz_read)
    p_write = scalar(P_freq_uW_MHz_write)
    k_read = scalar(K_lut_read)
    k_write = scalar(K_lut_write)
    if str(k_read).strip().upper() == "TBC" or str(k_write).strip().upper() == "TBC":
        print("Note: Memory configuration yet to be calibration. Default calibration parameters are being used.")
        k_read = 0.5018
        k_write = 0.4501
    energy_read = (float(p_read) * Clock_Frequency_Hz / 1e12 * float(k_read) * F_actbitlines * F_non_zeros_r) * SRAM_read_latency
    energy_write = (float(p_write) * Clock_Frequency_Hz / 1e12 * float(k_write) * F_actbitlines * F_non_zeros_w) * SRAM_write_latency
...
def update_ddr_mem_latency(mem_df, compute_json_data, tile_latency, tile_latency_breakdown, mem_latency, tile_map, DDR_access_table, mem_req, mem_size):
    #import pdb; pdb.set_trace()
    ddr_mem_req = sum([(v["DDR_accesses"])*v["DDR_bus_width"] for k, tile_dict in tile_latency_breakdown.items()  for v in tile_dict.values() if "DDR_accesses" in v])
    chip_tile_idx=tile_map["DDR_Mem"]
    mask = (mem_df["Chiplet ID"] == chip_tile_idx.split('_')[0]) & (mem_df["Tile ID"]   == chip_tile_idx.split('_')[1])
    mem_req[chip_tile_idx] = {k: sum(v["DDR_accesses"] * v["DDR_bus_width"] for v in tile_dict.values() if "DDR_accesses" in v) for k, tile_dict in tile_latency_breakdown.items()}
    ...
    mem_df.loc[mask, "mem_req_tn"]=ddr_mem_req
    ...
    tile_latency_breakdown[chip_tile_idx]={0:{"SRAM_read_latency": SRAM_read_latency, 
                                            "SRAM_write_latency": SRAM_write_latency}}     
    #print(chip_tile_idx,row["Chiplet ID"],row["Tile ID"])
    tile_latency[chip_tile_idx]=SRAM_read_latency+SRAM_write_latency
    mem_latency[chip_tile_idx]=tile_latency[chip_tile_idx]
