...
for model in aimodels:
    for set_suff_bank in SET_SUFF_BANKS:
        for Nbank in def_Nbank:
            for create in DEFAULT_FILES_GENERIC:
                if create=="True":
                    for type in TYPE_DEFAULT_FILES:
                        ...
                        with open('config.py', 'w') as f:
                            for line in lines:
                                if line.strip() != f"aimodel='{model}'" and line.strip() != f'TYPE_DEFAULT_FILES="{type}"' and line.strip() != f"DEFAULT_FILES_GENERIC={create}" and line.strip() != "" and line.strip() != f'SET_SUFF_BANKS={set_suff_bank}' and line.strip() != f'def_Nbank={Nbank}':
                                    f.write(line)
                    #import pdb; pdb.set_trace()
                else:
                    for idx, _ in enumerate(stack_count):
                        ...
                        with open('output_summary.txt', 'a') as f:
                            f.write(f"\n")
                            f.write(f"\n")
                            f.write(f"---------------------Model: {model}, Type: User-Defined, Stack Count: {stack_count[idx]}, Chip Count: {chip_count[idx]}, Nbank: {Nbank} -------------------------\n") 
                        #read config.py file and append lines to it
                        with open('config.py', 'a') as f:
                            f.write(f"\naimodel='{model}'\n")
                            f.write(f'DEFAULT_FILES_GENERIC={create}\n')
