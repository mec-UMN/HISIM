...
from Module_Compute.HISIM_2_0_Files.Mem import *
from Module_Compute.HISIM_2_0_Files.SA import *
from Module_Compute.HISIM_2_0_Files.CPU import *
...
def mem_requirement(mem_df, G_ai_model, G_chip,tile_map):
    mem_req={}
    ...
    for idx, row in mem_df.iterrows():
        ...
        #print(row)
        mem_size[layer_idx+"_"+mem_place[0]]=row["NB"]*row["NW"]*row["Nbank"]*row["prec"]
        mem_df.loc[idx, "Accesses_r"]=math.ceil(mem_req.get(layer_idx+"_"+mem_place[0],0)/row["NB"])
        mem_df.loc[idx, "Accesses_w"]=math.ceil(mem_req.get(layer_idx+"_"+mem_place[0],0)/row["NB"])
        mem_df.loc[idx, "DDR_access"]=mem_size[layer_idx+"_"+mem_place[0]]<mem_req.get(layer_idx+"_"+mem_place[0],0)
        ...
        if mem_place[0] == "W":
            mem_df.loc[idx, "DDR_access"]=True
        elif len(list(G_ai_model.out_edges(layer_idx)))== 0:
            mem_df.loc[idx, "DDR_access"]=True
                

        if mem_df.loc[idx, "DDR_access"]:
        ...
        mem_df.loc[idx, "mem_req"]=mem_req.get(layer_idx+"_"+mem_place[0],0)
        tile_map[layer_idx+"_"+mem_place[0]]=chip_tile_idx
    ...
    [DDR_access_table.add_row([k.split("_")[0], k.split("_")[1], tile_map.get(k,0).split("_")[0], tile_map.get(k,0).split("_")[1], v*1E-6, mem_req.get(k,0)*1E-6, mem_df[(mem_df["Chiplet ID"] == tile_map.get(k,0).split("_")[0]) & (mem_df["Tile ID"] == tile_map.get(k,0).split("_")[1])]["DDR_access"].values[0]]) 
    for k,v in mem_size.items()]
    #print(DDR_access_table)
    #import pdb; pdb.set_trace()
    return DDR_access_table, tile_map, mem_req
...
def load_compute_tiles(G_ai_model, G_chip):
    ...
    return cpu_df, tile_map, mem_df, sa_df, mapping_df, lut_df


def plot_component(areas, G_chip, noc_positions, filename, spacing=0.5, annotate=True):
    #print(areas, noc_positions)
    coords = {k: tuple(map(int, v.split(','))) for k, v in noc_positions.items()}
    ...
    for comp in areas:
        ...
        ax.add_patch(Rectangle((x, y), s, s, fill=True, color="skyblue"))
        if annotate:
            ax.text(x+s/2, y, comp+"\n"+G_chip.nodes[comp]["HW Type"], ha='center', va='bottom', fontsize=7)
        # Update bounds
        min_x = min(min_x, x )
        max_x = max(max_x, x + s)
        min_y = min(min_y, y )
    ...
    ax.set_title(f"Chip Map {filename.split("_")[-1].split(".")[0]}")
...
def compute_main_fn(G_ai_model, G_chip, tile_ids):
    ...
    cpu_df, tile_map, mem_df, sa_df, mapping_df, lut_df = load_compute_tiles(G_ai_model, G_chip)
    DDR_access_table, tile_map, mem_req=mem_requirement(mem_df, G_ai_model, G_chip, tile_map)
    ...
    print("Chip Area is:", sum(tile_area.values()), "mm2")
    ...
    if DEBUG:
        ...
        for chip_idx, tile_idx_dict in tile_ids.items():
            ...
            chip_noc_positions= {k: v for k, v in nx.get_node_attributes(G_chip, "NoC Position").items() if k in chip_tile_idx_list}        
            plot_component(chip_tile_area, G_chip, chip_noc_positions , filename)
        #plt.show()
    #import pdb; pdb.set_trace()
    #print(tile_area)
    return G_chip, tile_map, mem_req
