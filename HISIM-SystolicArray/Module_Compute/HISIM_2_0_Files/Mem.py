...
def latency_ddr(compute_json_data, row ):
    ...
    return latency_rd, latency_wr, N_raccess_rd+N_raccess_wr
...
def latency_mem_tile(mem_df, compute_json_data, tile_latency,tile_latency_breakdown):
    ...
    for _, row in mem_df.iterrows():
        ...
        if row["DDR_access"]:
            latency_rd_ddr, latency_wr_ddr, N_accesses_ddr=latency_ddr(compute_json_data, row)
            tile_latency_breakdown[chip_tile_idx].update({"DDR_read_latency": latency_rd_ddr, "DDR_write_latency": latency_wr_ddr, "DDR_accesses": (N_accesses_ddr)})
            ddr_latency[chip_tile_idx]=latency_rd_ddr+latency_wr_ddr
        else:
            tile_latency_breakdown[chip_tile_idx].update({"DDR_read_latency": 0, "DDR_write_latency": 0, "DDR_accesses": 0})
