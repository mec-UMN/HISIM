...
def load_ai_network(aimodel):
    ...
    elif aimodel=='resnet20':
        network_params = np.loadtxt(f'{target_file_path}/ResNet/20/NetWork.csv', dtype=int, delimiter=',')
...
def model_mapping(filename,placement_method,network_params,SA_size,N_arr,N_pe,N_tile,N_tier,N_stack,bit_width):
    ...
    with open(filename, 'w') as csvfile: 
        ...
        for layer_idx in range(0, total_number_layers):      
            ...
            total_MACs=n_c_x*n_c_y*SA_size*SA_size                   #Total number of MAC units in the mapped PEs for the layer
            total_weights=in_channel*k_x*k_y*out_channel             #Total number of weights for the layer
            utilization=total_weights/total_MACs                     #Cell Bit Utilization for the layer
            util_row=out_channel/(n_c_y*SA_size)                     #Average Utilization of a row for the layer
            util_col=in_channel*k_x*k_y/(n_c_x*SA_size)              #Average Utilization of a column for the layer
            ...
            #9-Tier index that the layer is mapped to for this layer,
            #10-Utilization for the layer,
            #11-Average Utilization of a row for the layer,
            #12-Total number of weights for the layer,
