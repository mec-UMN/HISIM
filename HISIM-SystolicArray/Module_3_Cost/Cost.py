...
def cost_main_fn(G_chip, G_sys, nw_df, stack_area, stack_ids, mesh_size, ip_list, n_signal_IO):
    ...
    for chip_idx in G_sys.nodes():
        if G_sys.nodes[chip_idx]["Chiplet ID"]!="Empty":
            ...
            cost_breakdown["die"][chip_idx] , yield_breakdown["die"][chip_idx] = cost_die(cost_params["k_silicon"], A_chip, cost_params["critical_area_ratio"], cost_params["A_reticle_unit"], cost_params["D0_chip"], cost_params["alpha_chip"], cost_params["litho_percent"], cost_params["Y_perstitch"], cost_params["Y_wafer_process"],  manuf_vol)
            cost_breakdown["test"]["wafer_probe"][chip_idx] = cost_wafer_probe(cost_params["C_ate_ws"], cost_params["C_probe_ws"], cost_params["C_site_ws"], yield_breakdown["die"][chip_idx], cost_params["escaperate_die_ws"], cost_params["T_per_insert_list_ws"], cost_params["T_reattempt_list_ws"], manuf_vol)
    ...
    for stack_idx, A_bond in stack_area.items():
        if A_bond != 0:
            ...
            cost_breakdown["assembly"][stack_idx] , yield_breakdown["assembly"][stack_idx]  = cost_assembly(cost_params["k_place"], cost_params["T_place"], cost_params["G_place"], cost_params["k_bond"], cost_params["T_bond"], cost_params["G_bond"],
            ...
            #import pdb; pdb.set_trace()
            cost_breakdown["test"]["final_package"][stack_idx]= cost_ftl(cost_params["C_ate_ftl"], cost_params["C_probe_ftl"], cost_params["C_site_ftl"], Y_stack , cost_params["escaperate_die_ftl"], cost_params["escaperate_die_ws"], cost_params["T_per_insert_list_ftl"], cost_params["T_reattempt_list_ftl"], manuf_vol)

    A_sub =1
    A_interposer =0
    if len(stack_area)>1:
        A_interposer = sum(stack_area.values())*cost_params["si_scaling_factor"]
        cost_breakdown["interposer"]["Si0"], yield_breakdown["interposer"]["Si0"] = cost_interposer_silicon(cost_params["C_wafer"], A_interposer, cost_params["critical_area_ratio_interposer"], 1, cost_params["r_wafer"], cost_params["D0_interposer_silicon"], cost_params["alpha_interposer_silicon"], cost_params["Y_wafer_silicon"], cost_params["A_reticle_unit_interposer"], cost_params["Y_perstitch_interposer"], manuf_vol)
        A_sub= cost_params["si_scaling_factor"]

    A_sub *= sum(stack_area.values())*cost_params["sub_scaling_factor"]
    N_sub = 1
    cost_breakdown["substrate"]["Sub0"] , yield_breakdown["substrate"]["Y_sub"] = cost_substrate(cost_params["k_substrate"], N_sub, cost_params["D0_sub"], A_sub, cost_params["critical_area_ratio_sub"], cost_params["alpha_sub"], cost_params["Y_wafer_sub"], manuf_vol)
    cost_breakdown["test"]["system_level"]["System0"] = cost_wafer_stl(cost_params["C_ate_stl"], cost_params["C_probe_stl"], cost_params["C_site_stl"], min(list(yield_breakdown["die"].values())) if len(yield_breakdown["die"].values())>0 else 0, cost_params["escaperate_die_ftl"], cost_params["T_per_insert_list_stl"], manuf_vol)
