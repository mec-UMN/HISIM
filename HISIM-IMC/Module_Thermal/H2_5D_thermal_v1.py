...
class H2_5D(object):
    def __init__(self,area_single_tile,single_router_area,chiplet_num,mesh_edge,area_aib,area_emib):
        #====================================================================================================
        # parameters
        #====================================================================================================
        self.alpha=3.5
        ...
        self.dict_z['heatsink']=40/1000
        self.dict_z['heatspread']=20/1000
        self.dict_z['subs']=1/1000
        self.dict_z['pcb']=1.6/1000
        self.dict_z['air']=50/1000
        self.heatsinkair_resoluation=0.5/1000
    ...
    #====================================================================================================
    def load_power_tier(self,power_l,router_l, tier_shape,power_id_tocood  ):
    ...
    def load_power_aib(self,power_aib_l,aib_shape, single_aib_size):
        ...
        if self.Nstructure==2: numofaib = 2
        elif self.Nstructure==3: numofaib = 4
        elif self.Nstructure==4: numofaib = 6
        else: 
            print('error')
            exit()
    ...
    def load_power_emib(self,power_emib_l,emib_shape, single_emib_size, aib_shape, single_aib_size):
        if self.Nstructure==2: numofemib = 1
    ...
    #====================================================================================================
    def create_global_structure(self,power_tier_l,power_router_l, power_aib_l, power_emib_l):
        #====================================================================================================
        # tier_container_l = []
        # power_id=0
        # for i in range(self.Nstructure):
        ...
        # load_power
        #====================================================================================================
        tier_p_container_l                    = self.load_power_tier(power_tier_l,power_router_l,single_tier_k.shape, power_id_tocood)
        ...
        # tier_p_container_l = load_power_tier(power_tier_l,power_router_l,single_tier_k.shape, power_id_tocood)
        # aib_p_container_l  = load_power_aib(power_aib_l,single_aib_k.shape, single_self.aib_size)
        # emib_p_container_l = load_power_emib(power_emib_l,single_emib_k.shape, single_self.emib_size)

        # pcb_p_tier = np.array([0]*N*N*2*2*self.emib_sublayer).reshape(self.emib_sublayer,N*2,N*2)
        # emib_p_aib  = np.array([0]*N*2*self.aib_width_n*self.emib_sublayer).reshape(self.emib_sublayer,N*2,self.aib_width_n)
        # air_p_emib = np.array([0]*N*2*self.emib_width_n*self.tier_sublayer).reshape(self.tier_sublayer,N*2,self.emib_width_n)
        ...
        pcb_height_l  = [self.heatsinkair_resoluation]*int((self.dict_z['pcb'])//self.heatsinkair_resoluation)
        ...
        pcb_name_l  = ['pcb']*int((self.dict_z['pcb'])//self.heatsinkair_resoluation)
        ...
        for layer in range(len(t)):
            ...
            for row in range(t.shape[1]):
                ...
                for col in range(t.shape[2]):
                    ...
                    if   boxx==self.imc_size and boxy==self.imc_size:   numofwidthblock = 2 ; numoflengthblock = 2
                    elif boxx==self.imc_size and boxy==self.r_size:     numofwidthblock = 2 ; numoflengthblock = 1
                    elif boxx==self.r_size and boxy==self.imc_size:     numofwidthblock = 1 ; numoflengthblock = 2
                    elif boxx==self.r_size and boxy==self.r_size:       numofwidthblock = 1 ; numoflengthblock = 1
...
"""
case_H2_5D=H2_5D(area_single_tile=0.336536,single_router_area=0.019626,chiplet_num=3,mesh_edge=3)
#====================================================================================================
...
G_sparse = case_H2_5D.get_conductance_G_new(full_k, grid_size, all_height_l, all_z_count_l)

case_H2_5D.solver(G_sparse, full_p,  all_z_count_l,grid_size)
"""
