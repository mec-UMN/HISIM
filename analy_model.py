...
import collections
from util import *
if not os.path.exists('./to_interconnect_analy'):
    os.makedirs('./to_interconnect_analy')
if not os.path.exists('./result_thermal'):
    os.makedirs('./result_thermal')
if os.path.exists('./result_thermal/1stacks'):
    shutil.rmtree('.//result_thermal/1stacks')
os.makedirs('.//result_thermal/1stacks')
#Take all below parameters as argument

xbar_size = 512 # 64,128,256,512,1024
N_tile=81 # 4,9,16,25,36,49 # how many tile in tier (chiplet)
...
start = time.time()
#network_params = np.loadtxt('./Networks/Transformer/VIT_base.csv', dtype=int, delimiter=',')
#network_params = np.loadtxt('./Networks/GCN/NetWork.csv', dtype=int, delimiter=',')
#network_params = np.loadtxt('./Networks/ResNet/50/NetWork.csv', dtype=int, delimiter=',')
#network_params = np.loadtxt('./Networks/ResNet/110/NetWork.csv', dtype=int, delimiter=',')
#network_params = np.loadtxt('./Networks/DenseNet_IMG/NetWork_121.csv', dtype=int, delimiter=',')
network_params = np.loadtxt('./Networks/VGG/VGG16_IMG/NetWork.csv', dtype=int, delimiter=',')
sim_name="GCN_1stack_placement_1"
#network_params = np.loadtxt('./Networks/Testing/NetWork.csv', dtype=int, delimiter=',')
...
tier_index=0
numComputation=0
filename = "./to_interconnect_analy/layer_inform.csv"
...
#---------------------------------------------------------------------#
placement_method=4 # 1:from the top to the bottom tier
...
with open(filename, 'w') as csvfile1: 
    ...
    for layer_idx in range(len(computing_data)):
        ...
        # single PE
        L_bus=(1.17733e-08+1.48273e-07)*xbar_size/64*computing_data[layer_idx][5]/392*64/bus_width*math.sqrt(N_crossbar)/2
        L_PE=L_single_crossbar+1.2472e-07*computing_data[layer_idx][5]/392*math.sqrt(N_crossbar)/2+(1.85E-7+1.25e-8)*computing_data[layer_idx][5]*math.sqrt(N_crossbar)/400+L_bus

        #print(L_PE-L_single_crossbar)
        #print(E_single_crossbar)

        L_accum=2.74e-09*math.sqrt(N_pe)/2 #unit=s
        if relu==True:
            L_activation=4e-9
        else:
            L_activation=1.64e-8
        L_hTree=2.82e-9*math.sqrt(N_pe)/2*quant_act/8*computing_data[layer_idx][5]/200
        L_buffer=(1.63551e-08+1.82825e-09)*computing_data[layer_idx][5]/392*quant_act/8
        ...
        L_accum_chip=3.97e-8*network_params[layer_idx][0]*network_params[layer_idx][1]/49
        ...
        E_PE=computing_data[layer_idx][1]*N_pe*(0.86E-8*computing_data[layer_idx][5]/200+(1.52798e-09+1.92434e-08)*computing_data[layer_idx][5]/400/64*xbar_size+(1.02465e-10+2.32079e-10)*computing_data[layer_idx][5]/400*xbar_size/64)
        
        E_tile=computing_data[layer_idx][1]*(5.18E-12*math.sqrt(N_pe)/2*xbar_size/64/200+(1.39E-09+2.12E-10)*computing_data[layer_idx][5]/200+1.14E-10+2.13E-09*computing_data[layer_idx][5]/200*math.sqrt(N_pe)/2)
        
        E_single_tile=N_pe*E_single_crossbar+N_pe*(0.86E-8*computing_data[layer_idx][5]/200+(8.39E-11+2.02E-10)*computing_data[layer_idx][5]/200/64*xbar_size+(7.8E-10+9.8E-9)*computing_data[layer_idx][5]/200*xbar_size/64)
...
for i in range(len(tile_total)-1):
    ...
    if routing_method==1:
        ...
        if tile_total[i][0][2]!=tile_total[i+1][0][2]:
            for x in range(len(tile_total[i])-1):
                for y in range(int(len(empty_tile_total[tile_total[i][x][2]])*percent_router)):
...
if len(tier_2d_hop_list)!=1:
    ...
    tier_3d_hop_list[-1]=27.454949999999997
tier_2d_hop_list_power=[i * single_2d_router_power/i*fclk_noc for i in tier_2d_hop_list]
tier_3d_hop_list_power=[i * single_tsv_channel_power/i*fclk_noc for i in tier_3d_hop_list]
#import pdb;pdb.set_trace()
print(tier_2d_hop_list_power)
print(tier_3d_hop_list_power)
print("2D NoC W2d",W2d)
print("3D TSV W3d",W3d)
print("network power",sum(tier_2d_hop_list_power)+sum(tier_3d_hop_list_power),"mW")
...
print("The sim time is:", (end_noc - start))
...
for i in range(len(tier_2d_hop_list_power)):
    if placement_method==1 or placement_method==3 or placement_method==4:
